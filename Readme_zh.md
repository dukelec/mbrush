...
需要传输的是 dptz 私有格式文件，图片转换方法见本文后续章节。

文件传输有关寄存器有以下：

 - dptz_rx 为已经传输的 dptz 文件大小，接收时解压缩，并把解压后的数据写入 psram
 - psram_f_start 到 psram_f_end 为已写入的文件头尾地址，相等表示无打印文件
...
`data` 为 dptz 文件切片，单次传输最多 251 字节。

临时端口号：
 - Bit3 为 1 时该端口不回复，为 0 正常回复
 - Bit[2:0] 为自增序号 cnt，每个 dptz 文件的首包序号都为 0，如果序号出错，则置位错误标记，并丢弃数据

修改 p14_cnt / p14_err 的方式：
 - 直接读写 p14_cnt / p14_err 寄存器
 - 使用 d_ctrl 寄存器清零
 - 0x14 端口写入 1 个 data 长度为 0 的空包：清除可恢复的错误（err_flag 1 和 2），不修改 p14_cnt

回复：
 - err_flag (bit[5:0]):
   - 为 0 时表示没有错误
   - 为 1 时表示 cnt 错误，错误清除前丢弃后续写入的数据
   - 为 2 时表示 dptz 文件头有错误
   - 为 3 时表示数据损坏无法解压，要写 0x01 到 d_ctrl 清除已经传输的 dptz 内容
 - err_flag bit7 为 1 时表示这是空包的回复（err 位为清除前的值）
 - p_state 为打印机当前状态
 - enc_val 是位置编码器当前计数
 - psram_w_offset 是当前数据包写入成功时的 psram_w_offset 值
 - dptz_rx_32 是当前数据包接收成功时的 dptz_rx 值
...
每个命令都要求回复可以确保数据正确写入，但是传输效率会很低，为了提高传输效率，可使用以下方式：
 ...
 3. 收到错误回复或等待回复超时：停止发送，写入 1 个空包，然后丢弃收到的回复，
    直到收到 err_flag bit7 置位的回复——回复按顺序发出，它必然是最后一条，之后不会再有残留的旧回复。然后：
    - err 位为 3：写 0x01 到 d_ctrl，然后重传整个文件
    - 否则：从回复中的 dptz_rx 处续传（该位置总是对齐到数据包边界，0 表示从头重传）
